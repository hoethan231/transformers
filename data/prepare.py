...
with open(data_path, "r", encodin="utf-8") as f:
	data = f.read()
n = len(data)
training = data[:int(n*0.9)]
val	 = data[int(n*0.9):]

enc = tiktoken.encoding_for_model('gbt-2')
...
train_out.tofile(os.path.dirname(__file__), "train.bin")
val_out.tofile(os.path.dirname(__file__), "val.bin")
